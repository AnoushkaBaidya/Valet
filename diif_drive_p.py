"""  DIFFERENTIAL DRIVE SKID ROBOT VALET """
...
class Robot:
    def __init__(self) -> None:
        ...
        self.veh_width = 30
        self.veh_height = 20
        #Positioning teh vehicles in the grid
        ...
        self.ego_veh_end = [self.l_vehicle[0] + self.veh_width + 15, 30 + self.veh_height/2,0]
...
class World:
    ...
    def environment(self,x,y,theta):
        ...
        obs_p = [self.obs[0]+63, self.obs[1]+26.5]
        v1_p = [50, 0.8]
        v2_p = [155,0.8]  
...
class Planner():
    ...
    def get_neighbours(self, x, y, theta):
        neighbours = []
        v_right = self.robot.v_right
        v_left = self.robot.v_left

        for i in range(len(v_right)):
            for j in range(len(v_left)):
                if v_right[i] == 0 and v_left[j] == 0:
                    continue
                
                vel = (v_right[i] + v_left[j]) / 2
                delta_theta = (self.robot.robot_wRadius / (2 * self.robot.robot_wBase)) * (v_left[j] - v_right[i])*(180/math.pi)
                theta_new = (theta + delta_theta) 
                x_new = x + vel * math.cos(theta * math.pi / 180)
                y_new = y + vel * math.sin(theta * math.pi / 180)

                if self.valid_point(x_new, y_new, theta_new):
                    neighbours.append([round(x_new, 2), round(y_new, 2), round(theta_new, 2)%360, 1, v_right[i]])
    ...
    def path_available(self, x, y):   # Define boundary line segment
        ...
        # Check for collisions with obstacles and other vehicles
        obstacles = [self.world.obstacle, self.world.vehicle1]
    ...
    def AStar_path_planner(self):
        ...
        while not prior_queue.empty():
            #Inside the loop, the node with the  f value is retrieved from the priority queue and its coordinates, g_score, f_score and path are extracted.
...
if __name__ == "__main__":
    Diff_rob = Robot()
    world = World(Diff_rob)
    Diff_controller = Controller(Diff_rob,world)
    Diff_planner = Planner(Diff_rob,world,Diff_controller) 
    path_values_calculated = Diff_planner.AStar_path_planner()
...
plt.figure("Path taken by the Differential Drive Robot ")
plt.xlim([0,110])
plt.ylim([-20,200])
x_values = [point[0] for point in path_values_calculated]
y_values = [point[1] for point in path_values_calculated]
